...
public class AuthController {
    ...
    public HttpEntity<?> addDoctor(@RequestBody RegisterDto registerDto, @PathVariable UUID id) {
        ApiResponse<?> apiResponse = authService.addDriver(id, registerDto);
        return ResponseEntity.status(apiResponse.getStatus()).body(apiResponse);
    }

    @DeleteMapping("/doctor/{id}")
    public HttpEntity<?> deleteDoctor(@PathVariable UUID id) {
        ApiResponse<?> apiResponse = authService.deleteDriver(id);
        return ResponseEntity.status(apiResponse.getStatus()).body(apiResponse);
    }

    @GetMapping("/driver")
    public HttpEntity<?> getDriver() {
        List<User> doctor = authService.getDriver();
